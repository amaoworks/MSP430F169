...
//{
//    Address_set(x,x,y,y);                                                //横屏这里要进行坐标变换，如果是竖屏，则直接是x0,x1,y0,y1做参数
//
...
void GUIDotline(uint x0,uint y0,uint x1,uint y1,uint line_color)   //画虚线函数
{
    ...
    for(i = 0;i < Dx;i ++)
    {
        if((i%10)<=4) Put_pixel(x0,y0,line_color);       //画点，虚线间距大小可以在这里调节，设置不同的判断条件
        if(sub >= 0)
        {
            if(status == 1)                                         //在靠近Y轴区，x值加1
                x0 += s1;
...
{
    Put_pixel(xc+x,yc+y,yc_colour);
    Put_pixel(xc+x,yc-y,yc_colour);
    Put_pixel(xc-x,yc+y,yc_colour);
    Put_pixel(xc-x,yc-y,yc_colour);
    Put_pixel(xc+y,yc+x,yc_colour);
    Put_pixel(xc+y,yc-x,yc_colour);
    Put_pixel(xc-y,yc+x,yc_colour);
    Put_pixel(xc-y,yc-x,yc_colour);
}
