...
//加1上偏移更大，减去1下偏移小一点，可以使用
uint chx=3898,chy=145;//默认像素点坐标为0时的AD起始值

#define Xmin           0x0133
#define Xmax           0x0EDA
#define Ymin           0x00D0
#define Ymax           0x0E88       //修改处
...
{
    P2SEL = 0x00;
    P2DIR |= BIT4 + BIT5 + BIT6;       //触摸控制线，P60~P64，2条输入线，4条输出线
...
uchar pix_filter(struct struct1 pix1,struct struct1 pix2)
{
    ...
    if(x<10&&y<10)                                    //软件滤波，2次取样的值相差太大的视为噪声
    {
        ...
        coordinate.x=(pix1.x+pix2.x)>>1;           //求两次采样平均值
        coordinate.y=(pix1.y+pix2.y)>>1;
    }
    return flag;
}
        ...
        if(pix_filter(pix1,pix2)==1) //得到当前TP的取样值，此处使用软件滤波，2次取样的值相差太大的视为噪声
        {
            if((coordinate.x>Xmin)&&(coordinate.y>Ymin))
            {
                //                    lx=(uint)(240.0*(coordinate.x-Xmin)/(Xmax-Xmin));       //坐标转换，即根据采样值计算实际坐标值
                                    ly=(uint)(320.0*(Ymin-coordinate.y)/(Ymax-Ymin));   //Xmin、max和Ymin、Ymax分别是触摸屏横纵坐标的最小/最大值
                lx=coordinate.x>chx?((ulong)coordinate.x-(ulong)chx)*1000/vx:((ulong)chx-(ulong)coordinate.x)*1000/vx;
//                ly=coordinate.y<chy?((ulong)coordinate.y-(ulong)chy)*1000/vy:((ulong)chy-(ulong)coordinate.y)*1000/vy;
                flag=1;
            }
        }
    }
